...
- Одна или несколько строк; каждая строка: `product; source; amount[; +|вид деятельности]`.
- **Расход:** сумма без префикса, например `Еда; Лента; 1755.5`.
- **Доход:** сумма с ведущим `+` в поле суммы, например `Зарплата; Работа; +50000`.
- Четвёртое поле **`+`** — флаг «woman» (колонка Woman / `+`); **любой другой текст** — вид деятельности (колонка Work, напр. `Менторство`, `Личный проект`). В тексте деятельности нельзя использовать `;`.
...
- Внутри: секции `## *Spending:*` и `## *Income:*`, markdown-таблицы; при наличии woman и/или work — колонки **Woman** и **Work** (Work — произвольный текст).
- Дашборд Obsidian: `00 - Dashboard cash flow.md` — фильтры «Женщина» / «Работа» (взаимоисключающие); при фильтре «Работа» в таблице показывается колонка «Деятельность».
...
- Колонка **Work** — произвольный текст «вида деятельности» (`Сервер; Hetzner; 1200; Менторство`); `+` по-прежнему woman. Фильтр «Работа» на дашборде — все строки с непустой Work; при фильтре колонка «Деятельность» в таблице. Файлы: `bot.py`, `bot_server.py`, `receiver.py`, тесты, `00 - Dashboard cash flow.md`.
- Флаг **work** (`; -`): первая итерация — заменена на текстовую Work (см. выше).
